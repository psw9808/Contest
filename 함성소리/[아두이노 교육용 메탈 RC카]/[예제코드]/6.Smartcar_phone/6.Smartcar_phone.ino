...
#include <AFMotor.h>
AF_DCMotor motor_L(1);              // 모터드라이버 L293D  3: M3에 연결,  4: M4에 연결
...
void loop(){
  ...
  else if(rec_data == 'd'){ // 후진, back
     motor_L.run(BACKWARD);  motor_R.run(BACKWARD);    
  }
  else if(rec_data == 'b'){ // 좌회전, Go Left
   motor_L.run(RELEASE);  motor_R.run(FORWARD);     
  }
  else if(rec_data == 'c'){ // 우회전, Go Right
    motor_L.run(FORWARD);  motor_R.run(RELEASE);                
  }
  else if(rec_data == 'g'){ // 제자리 회전, Right Rotation
     motor_L.run(BACKWARD);   motor_R.run(FORWARD);      
  }
  else if(rec_data == 'e'){ // 제자리 회전, Left Rotation
